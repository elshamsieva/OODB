...
import ru.icmit.oodb.dz.lab1.Metro;
...
public class LoadDB {
    ...
    public static Metro load() throws IOException {
        Metro metro = null;
        String metroStr = "";
        File file = new File("metro.json");

        if (file.exists()) {
            metroStr = new String(Files.readAllBytes(file.toPath()));
        }

        metro = new Gson().fromJson(metroStr, Metro.class);
